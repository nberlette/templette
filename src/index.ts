...
export type Values<K extends string | number = string, V = string | ((...args: string[]) => string)> =
  | {[P in K]: V}
  | [K, V][]
  | V[];
...
export default class Templette {
  ...
  static compile(template: string, values: Values): string {
    return template.replace(/[{]{1,3}\s*(.*?)\s*[}]{1,3}/g, (x: string | number, key: string | any, y: any) => {
      (x = 0), (y = values);
      key = key.trim().split('.');
  ...
  static cleanup(template: string, substitutions?: [string | RegExp, any][]): string {
    substitutions = substitutions || [[/[{]{1}[\s ]?([\S]+)[\s ]?[}]{1}/g, (_m0: string, m1: string) => `{{${m1}}}`]];
